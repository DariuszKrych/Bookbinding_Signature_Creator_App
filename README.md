...
*A cut-off reply does not arrive as a reply, though*, and that took a broken book to find out. The OpenAI client will not hand over a completion whose `finish_reason` is `length`: it raises `LengthFinishReasonError` and drops the words on the floor. Because the ledger below sends a `max_tokens` with **every** request, filling the cap is the ordinary way a full batch ends rather than a rare mishap — so the first book written after the token ceiling went in streamed four chapters onto the page and then replaced them with a red banner. `_is_cut_off` recognises that exception and `_cut_off_reply` takes the chapters back out of it, before anything downstream can mistake a short book for a broken one. A cut-off reply is also never *repaired*: a repair would carry a cap no bigger and run out in the same place, and it would spend a request the chapters after it need.
...
- **Truncation is also tested the way it really arrives** — as the exception the OpenAI client raises instead of the reply. A fake of that exception, built in the test file so nothing has to import the client, is fed both ways a real one lands: thrown before a word arrives, and thrown after a whole reply has already streamed onto the page. The book has to come back with five chapters both times, the words already on screen have to still be there, and the request a repair would have wasted has to have gone on the chapters that were left.
